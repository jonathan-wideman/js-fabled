...
import { elementToItem, elementToStartingAbilities, getEquippedItem, isEquipment, range, sectionTickCodeword } from "./helpers"
...
export function GameProvider({ children }) {
    ...
    const setupStartingCharacter = (name, profession) => {
        ...
        if (!startingCharacterData.isSuccess) { return }

        const adventurersData = new DOMParser().parseFromString(startingCharacterData.data, 'text/xml')
        // console.log(adventurersData)s
        const staminaMax = parseInt(adventurersData.getElementsByTagName('stamina')?.[0]?.getAttribute('amount')) || 0
        const rank = parseInt(adventurersData.getElementsByTagName('rank')?.[0]?.getAttribute('amount')) || 0
        const money = parseInt(adventurersData.getElementsByTagName('gold')?.[0]?.getAttribute('amount')) || 0
        // const abilities = adventurersData.querySelector(`profession[name="${profession}"]`)?.textContent?.split(' ')
        const abilities = elementToStartingAbilities(adventurersData.querySelector(`profession[name="${profession}"]`))

        const items = Array.from(adventurersData.querySelectorAll([
            `weapon[profession="${profession}"]`, `weapon:not([profession])`,
            `armour[profession="${profession}"]`, `armour:not([profession])`,
            `armor[profession="${profession}"]`, `armor:not([profession])`,
            `item[profession="${profession}"]`, `item:not([profession])`
        ].join(', '))).map(e => elementToItem(e))
        // console.log(items)
        // TODO: only equip single best items
        const equipped = items.map(item => ['weapon', 'armor'].includes(item.type) ? { ...item, equipped: true } : item)
        const extraItems = [
        ...
        setCharacter({
            ...
            money,
            inventory: { ...defaultCharacter.inventory, items: [...defaultCharacter.inventory.items, ...equipped, ...extraItems] },
            // titles: [],
            // god: undefined,
            // blessings: [],
            // curses: [],
    ...
    const giveCharacter = (type, value) => {
        switch (type) {
            ...
            case 'codeword':
                if (character.codewords.includes(value)) { return }
                setCharacter({ ...character, codewords: [...character.codewords, value] })
                break;
    ...
    const takeCharacter = (type, value) => {
        switch (type) {
            ...
            case 'codeword':
                if (!character.codewords.includes(value)) { return }
                setCharacter({ ...character, codewords: character.codewords.filter(codeword => codeword !== value) })
                break;
    ...
    const equipCharacter = (item) => {
        ...
        if (itemIndex >= 0) { newItems[itemIndex] = { ...item, equipped: true } }
        if (currentIndex >= 0) { newItems[currentIndex] = { ...current, equipped: undefined } }
        setCharacter({ ...character, inventory: { ...character.inventory, items: newItems } })
    }
    ...
    const getSectionTickedBoxes = (section) => {
        const s = section ?? page
        const max = sectionVars.boxes ?? 0
        const ticks = range(max).map((box, i) => characterHas('codeword', sectionTickCodeword(book, s, i)))
